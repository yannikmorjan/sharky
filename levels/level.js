function initLevel(lvl) {
    if (lvl == 'lvl1') {
        return level1
    }
}

const level1 = new Level(
    [
        new PufferFish(),
        new JellyFish(),
        new PufferFish(),
        new Endboss()
    ],
    [
        new Tunnel(0),
        new RockVertical(720),
        new RockHorizontal(2*720),
    ],
    [
        new BackgroundObject('img/3. Background/Layers/5. Water/D2.png', -720,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D1.png', 0,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D2.png', 720,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D1.png', 720*2,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D2.png', 720*3,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D1.png', 720*4,-3),
        new BackgroundObject('img/3. Background/Layers/5. Water/D2.png', 720*5,-3),

        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D2.png', -720,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D1.png', 0,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D2.png', 720,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D1.png', 720*2,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D2.png', 720*3,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D1.png', 720*4,-3),
        new BackgroundObject('img/3. Background/Layers/4.Fondo 2/D2.png', 720*5,-3),

        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D2.png', -720,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D1.png', 0,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D2.png', 720,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D1.png', 720*2,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D2.png', 720*3,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D1.png', 720*4,-1),
        new BackgroundObject('img/3. Background/Layers/3.Fondo 1/D2.png', 720*5,-1),

        new BackgroundObject('img/3. Background/Layers/1. Light/2.png', -720,-0.5),
        new BackgroundObject('img/3. Background/Layers/1. Light/1.png', 0,-0.5),
        new BackgroundObject('img/3. Background/Layers/1. Light/2.png', 720,-0.5),
        new BackgroundObject('img/3. Background/Layers/1. Light/1.png', 720*2,-0,5),
        new BackgroundObject('img/3. Background/Layers/1. Light/2.png', 720*3,-0,5),
        new BackgroundObject('img/3. Background/Layers/1. Light/1.png', 720*4,-0,5),
        new BackgroundObject('img/3. Background/Layers/1. Light/2.png', 720*5,-0,5),

        new BackgroundObject('img/3. Background/Layers/2. Floor/D2.png', -720,0),
        new BackgroundObject('img/3. Background/Layers/2. Floor/D1.png', 0,0),
        new BackgroundObject('img/3. Background/Layers/2. Floor/D2.png', 720,0),   
        new BackgroundObject('img/3. Background/Layers/2. Floor/D1.png', 720*2,0),
        new BackgroundObject('img/3. Background/Layers/2. Floor/D2.png', 720*3,0),
        new BackgroundObject('img/3. Background/Layers/2. Floor/D1.png', 720*4,0),
        new BackgroundObject('img/3. Background/Layers/2. Floor/D2.png', 720*5,0)
    ]
);